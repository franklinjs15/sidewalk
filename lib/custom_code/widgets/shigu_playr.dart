...
class _ShiguPlayrState extends State<ShiguPlayr> {
  ...
  void _fetchVideos() {
    FirebaseFirestore.instance
        .collection('videos') //Your video collection name here
        .orderBy('dateUploaded', descending: true)
        .snapshots()
        .listen((snapshot) {
      setState(() {
        _videos = snapshot.docs;
        _videos.shuffle(); // Shuffle the videos
        ...
        if (_videos.isNotEmpty && _currentPage < _videos.length) {
          final DocumentReference<Object?>? vidTempID =
              _videos[_currentPage]['videoIDPassable'] as DocumentReference<
                  Object?>?; //must match video document string field, advised do not change
          FFAppState().update(() {
            FFAppState().tempVidID =
                vidTempID; //must match app state variable, advised do not change
          });
        }
        if (_videos.isNotEmpty && _currentPage < _videos.length) {
          final DocumentReference<Object?>? creatorID =
              _videos[_currentPage]['creatorID'] as DocumentReference<
                  Object?>?; //must match video document string field, advised do not change
          FFAppState().update(() {
            FFAppState().tempCreatorID =
                creatorID; //must match app state variable, advised do not change
          });
        }

        if (_videos.isNotEmpty && _currentPage < _videos.length) {
          Map<String, dynamic>? videoData =
              _videos[_currentPage].data() as Map<String, dynamic>?;

          if (videoData != null &&
              videoData.containsKey(
                  'videoHash') && //must match video document array field, advised do not change
  ...
  void _onPageChanged(int index) {
    ...
    if (_videos.isNotEmpty && _currentPage < _videos.length) {
      ...
      if (videoData != null &&
        ...
        if (firstHash != null) {
          String? userRef =
              currentUserReference; // Get the current user's reference
          if (userRef != null) {
            // Add the hash to the user's userHash array in Firestore
  ...
  void _initializeController(int index) {
    ...
    _controllers[index] = VideoPlayerController.network(_videos[index]
        ['videoUrl']) //do not ignore deprecated field!! Leave alone
      ..initialize().then((_) {
        setState(() {});
      });
  }
          ...
          return Stack(
            children: [
              ...
              Align(
                ...
                child: Padding(
                  padding: EdgeInsetsDirectional.fromSTEB(10, 0, 0, 65),
                  child: Container(
                    width: 277,
                    height: 34,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 32), // Added padding here
                      Column(
                        children: [
                          Container(
                            width: 40,
                            ...
                            child: Image.network(
                              'https://storage.googleapis.com/flutterflow-io-6f20.appspot.com/projects/watch-it-tik-tok-clone-xyjz2w/assets/r0rphgoae1nn/icons8-like-104.png',
                              fit: BoxFit.cover,
                            ),
                          ),
                        ],
                      ...
                      Column(
                        ...
                        children: [
                          Container(
                            ...
                            child: Image.network(
                              'https://storage.googleapis.com/flutterflow-io-6f20.appspot.com/projects/tok-flow-01-h-e6-d7-x-w65-a-y-c4-q-y-s49-q-k-b-g8-e-49jj7l/assets/doc2m8ydws5w/sidewalk_research-sharethicky.png',
                              fit: BoxFit.cover,
                            ),
                          ),
                          Text(
